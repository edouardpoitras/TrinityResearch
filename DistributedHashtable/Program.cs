...
namespace DistributedHashtable
{
    class Program
    {
        ...
        {
            TrinityConfig.LoadConfig(args[1].Trim());

            if (args[0].Trim().ToLower().StartsWith("-s"))
            {
                //TrinityConfig.AddServer(new Trinity.Network.ServerInfo("127.0.0.1", port, Global.MyAssemblyPath, Trinity.Diagnostics.LogLevel.Error));
